...
import { Camera, Heart, LayoutDashboard, LogOut, Moon, ShieldCheck, Shirt, Sparkles, Sun, User } from 'lucide-react';
      ...
      <main className="lg:pl-64">
        <header className="sticky top-0 z-10 flex h-16 items-center justify-between border-b border-slate-200 bg-white/90 px-4 backdrop-blur dark:border-slate-800 dark:bg-slate-900/90 sm:px-6">
          ...
          <div className="flex items-center gap-2">
            {isAdminUser(user) && (
              <Link className="button-secondary px-3" to="/admin">
                <ShieldCheck size={18} /> <span className="hidden sm:inline">Admin</span>
              </Link>
            )}
        ...
        <nav className={`fixed inset-x-0 bottom-0 grid ${visibleNavItems.length === 6 ? 'grid-cols-6' : 'grid-cols-5'} border-t border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900 lg:hidden`}>
          {visibleNavItems.map((item) => (
